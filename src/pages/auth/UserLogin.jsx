  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Capture name from login response if provided by Auth server
        if (data.name) {
        ...
        try {
          ...
          } else if (profileApiResponse.status === 404) {
            // ONLY redirect to submit profile if it explicitly doesn't exist
            setTimeout(() => navigate("/user/submit-profile"), 1000);
          } else {
            // For other server errors, default to dashboard to avoid blocking the user
            setTimeout(() => navigate("/user/dashboard"), 1000);
          }
        } catch (error) {
          console.error("Error checking profile status (server might be offline):", error);
          // If the profile server is offline, proceed to dashboard
          setTimeout(() => navigate("/user/dashboard"), 1000);
        }
      } else {
        let errorData;
  ...
  const handleReset = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showMessage(typeof errorData === 'string' ? errorData : (errorData.error || errorData.message || "Reset failed"), "error");
        shakeForm();
      }
    } catch (error) {
